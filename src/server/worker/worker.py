...
def _executor(simulate_gpu, gpu_number, profiled_latencies,
    ...
    def _lazy_drop(metadata):
        ...
        if (model_time - remaining_time) > 5.0:
            return True, None
    ...
    try:
        while not close_event.is_set():
            ...
            if res[0] is not None:  # Model number
                if res[2] is not None:  # New active model
                    model_number, batch_size, model = res
                else:  # Only batch size change
                    model_number, batch_size = res[:-1]
                model_update_time = (time.time() - t1)*1e3
                print(f"model update time on gpu {gpu_number}: {model_update_time}"
                      f" for {model_number}, {batch_size}")
            ...
            if imgs.size != 0 and dummy_count != batch_size:
                ...
                idle_plus_exec_time = (idle_time_end - idle_time_start)*1e3
                predict_time = (idle_time_end - start_time)*1e3
                gpu_execution_time = predict_time - batch_time
                ...
                # Log to the logger. @TODO: Note that this operation is in the critical path. Make sure this is light weight
                stats = {
                    "index": index,
                    "active_model_number": model_number,
                    "active_batch_size": batch_size,
                    "requests_in_batch": max(0, batch_size - dummy_count),
                    "idle_plus_exec_time": idle_plus_exec_time,
        ...
        if _EXECUTOR_TYPE == "thread":
            self._executor_thread = threading.Thread(target=_executor,
                                                     args=(self.opts.simulate_gpu, self._gpu_number, self._profiled_latencies,
                                                           ...
                                                           self._active_model_queue, self._close_event, self.opts.log_path))
        else:
